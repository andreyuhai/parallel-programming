...
int main(int argc, char **argv)
{
 ...
 // Array to use as a buffer to send and receive messages.
 int *sendArr = calloc(comm_size, sizeof(int));
 // Array to keep messages from other processors that are meant for the running processor.
 ...
 */
 ...
 // Destination to send the message to which will always be the same in a ring network. Next processor.
 int destination = (taskId + 1) % comm_size;  
 
 // Source to receive the message from which will always be the same in a ring network. Previous processor.
 int source = taskId - 1;
 
 // Since it is a ring network if our source ID is negative (0 - 1 = -1 if the taskId is 0)
 // Then we are summing it with the number of processors which will give us the ID of the last processor in the ring.
 if(source < 0)
  source += comm_size;
